//从这一版本开始，需要基于 SaasTool了
//TODO 要把 函数转为使用 SaasTool的
...
var _func_create_action=function(index,action,action_disp){
	...
	return  '<a href="javascript:alert(720);" index="' + index + '" action="' + action + '" class="easyui-linkbutton mg-grid-action" onclick="' + _cancel_bubble_s + '">' + action_disp+ '</a>&nbsp;';
}
...
var clsSimpleGrid=function(params){
	...
	var _proc_mggrid_handledata = function (callback)
	{
		...
		if(me.params.fixCol){
			for(var i in table_columns[0]){
				var _col = table_columns[0][i];
				var _field = _col['field'];
				var _fix = me.params.fixCol[_field];
				if(_fix){
					table_columns[0][i] = $.extend({}, _col, _fix);
				}
			}
		}
		...
		//TODO 如果不是第一次的话，就只需要 datagrid('loadData'....)????
