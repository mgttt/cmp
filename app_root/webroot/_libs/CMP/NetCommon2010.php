...
//TODO remove memcached soon, using cache_disk instead~
...
class NetCommon {
	...
	public function setDebug($f){
		//throw new Exception NetCommon.setDebug deprecated
	}
	public function getConnInfo(){
		return $this->conn_info;
	}
	public function setMemCached($ip,$port){
		$this->_mem_ip=$ip;
		$this->_mem_port=$port;
		$memcache = new Memcache;
		$rt = $memcache->connect($this->_mem_ip, $this->_mem_port);
		if(! $rt) throw new Exception("NetCommon.setMemCached:failed to connect Memcache");
		$this->_mc = $memcache;
		$this->_flag_cache=true;
	}
	...
	public function saveSessionVar($key,$value){
		if($this->_mc==null){
			if($this->_cd!=null){
				//println("_cd save ".$this->_sess_key ." $key,$value");
				return $this->_cd->save($this->_sess_key ."_" .$key, $value,300);//5 min.
			}
			throw new Exception("NetCommon.saveSessionVar:Memcached not config");
		}
		return $this->_mc->set($this->_sess_key ."_" .$key, $value);
	}
	public function loadSessionVar($key){
		if($this->_mc==null){
			...
			throw new Exception("NetCommon.loadSessionVar:Memcached not config");
		}
		return $this->_mc->get($this->_sess_key ."_" .$key);
	}
	...
	public function deleteCookie() {
		if($this->_mc!=null) $this->_mc->delete($this->_sess_key ."_COOKIE");
		//else if no mc mode?
	}

	function __destruct() {
		if($this->_mc!=null) {$this->_mc->close(); $this->_mc=null;}
