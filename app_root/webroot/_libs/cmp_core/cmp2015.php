...
class cmp2015
{
	...
	public static function Run($_json_string){
		...
		if ( $json_string || self::is_array_and_not_empty(self::$_preSetData) ) {
		}else{
		...
		//NOTES 20151122: 设计的原意是如果提交的 json_obj里面有 c m p就原始标准用法
		if(!$request_method_param){
			if($json_obj){
				$request_method_param=array_merge(array(),$_REQUEST, $json_obj);//20151113.wj
				//NOTES 会有 _c/_m/class/method/saeut等，总之用不到就别理，要用到就精选，不要期待 _p一定就是完全提交上来的
			}else{
				$request_method_param=array_merge(array(),$_REQUEST);//20151113.wj
			}
		}else{
			//20151113.wj 兼容不起来,有些旧代码变态而吃不下.
	...
	//	$rt=array();
	//	try{
	//		//@deprecated .shtml 现已经不推荐用 .static 方法更优化{
